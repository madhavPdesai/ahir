-- $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/vhdsclibs/data/simprims/simprim/VITAL/X_RAMB16_S2.vhd,v 1.3 2010/01/14 19:43:23 fphillip Exp $
-------------------------------------------------------------------------------
...
-- /___/   /\     Filename : X_RAMB16_S2.vhd
-- \   \  /  \    Timestamp : Thu Apr  8 10:57:14 PDT 2004
--  \___\/\___\
--
-- Revision:
...
entity X_RAMB16_S2 is
  ...
  attribute VITAL_LEVEL0 of
    X_RAMB16_S2 :     entity is true;
end X_RAMB16_S2;

architecture X_RAMB16_S2_V of X_RAMB16_S2 is

  attribute VITAL_LEVEL0 of
    X_RAMB16_S2_V : architecture is true;

  signal ADDR_ipd : std_logic_vector(12 downto 0) := (others => 'X');
  ...
  signal ADDR_dly : std_logic_vector(12 downto 0) := (others => 'X');
...
begin
  ...
  WireDelay : block
  begin
    ADDR_DELAY       : for i in 12 downto 0 generate
  ...
  VITALBehavior : process
    ...
    variable DO_zd          : std_logic_vector(1 downto 0)               := INI(1 downto 0 );    
  ...
  begin
    ...
    if (GSR_dly = '1') then
      DO_zd := INI(1 downto 0 );
    elsif (GSR_dly = '0') then
      if (rising_edge(CLK_dly)) then
        if (EN_dly = '1') then
          if (SSR_dly = '1') then
            DO_zd := To_StdLogicVector(SRVAL)(1 downto 0);
          else    
            if (WE_dly = '1') then
              if (wr_mode = 0) then
                DO_zd := DI_dly;
