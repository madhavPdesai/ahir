-- $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/vhdsclibs/data/simprims/simprim/VITAL/X_RAMB16_S18.vhd,v 1.3 2010/01/14 19:43:23 fphillip Exp $
-------------------------------------------------------------------------------
...
entity X_RAMB16_S18 is
  ...
  attribute VITAL_LEVEL0 of
    X_RAMB16_S18 : entity is true;
end X_RAMB16_S18;

architecture X_RAMB16_S18_V of X_RAMB16_S18 is

  attribute VITAL_LEVEL0 of
    X_RAMB16_S18_V : architecture is true;

  signal ADDR_ipd : std_logic_vector(9 downto 0)  := (others => 'X');
  signal CLK_ipd  : std_ulogic := 'X';
  signal DIP_ipd   : std_logic_vector(1 downto 0) := (others => 'X');
  ...
  signal ADDR_dly : std_logic_vector(9 downto 0)  := (others => 'X');
  signal CLK_dly  : std_ulogic := 'X';  
  signal DIP_dly   : std_logic_vector(1 downto 0) := (others => 'X');
  ...
  constant parity_width : integer := 2;
...
begin 
  ...
  WireDelay : block
  begin
    ADDR_DELAY       : for i in 9 downto 0 generate
    ...
    DIP_DELAY       : for i in 1 downto 0 generate    
  ...
  VITALBehavior : process
    ...
    variable DOP_zd : std_logic_vector(1 downto 0) := INI(17 downto 16  );
    variable DO_zd  : std_logic_vector(15 downto 0) := INI(15 downto 0 );    
  ...
  begin 
    ...
    if (GSR_dly = '1') then
      DO_zd := INI(15 downto 0 );
      DOP_zd := INI(17 downto 16 );
      
    elsif (GSR_dly = '0') then
      if (rising_edge(CLK_dly)) then
        if (EN_dly = '1') then
          if (SSR_dly = '1') then
            DO_zd := To_StdLogicVector(SRVAL)(15 downto 0);
            DOP_zd := To_StdLogicVector(SRVAL)(17 downto 16);          
          else    
            if (WE_dly = '1') then
              if (wr_mode = 0) then
                DO_zd := DI_dly;
