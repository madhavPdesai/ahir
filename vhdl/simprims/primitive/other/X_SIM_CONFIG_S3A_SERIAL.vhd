...
-- /___/   /\     Filename : X_SIM_CONFIG_S3A_SERIAL.vhd
-- \   \  /  \    Timestamp : 
--  \___\/\___\
--
-- Revision:
...
entity X_SIM_CONFIG_S3A_SERIAL is
  generic (
...
-- end_timing generic
  ...
  attribute VITAL_LEVEL0 of X_SIM_CONFIG_S3A_SERIAL : entity is true;
end X_SIM_CONFIG_S3A_SERIAL;

architecture X_SIM_CONFIG_S3A_SERIAL_V of X_SIM_CONFIG_S3A_SERIAL is
...
begin
    ...
    rdwr_b_in <= '0';
        ...
        begin
          if (DEVICE_ID = X"00000000") then
             assert FALSE report "Attribute Syntax Error : The attribute DEVICE_ID on  X_SIM_CONFIG_S3A_SERIAL is not set." severity error;
          end if;
          wait;
        end process;
      ...
      if (mode_sample_flag = '0') then
        if(prog_b_t = '1') then
           ...
           if (m_in /= "111") then
              ...
              Write ( Message, string'(" . Only Slave SERIAL mode M=111 supported on X_SIM_CONFIG_S3A_SERIAL."));
              assert false report Message.all severity error;
              DEALLOCATE (Message);
           end if;   
        elsif (NOW > 0 ps) then
            assert false report "Error: PROGB is not high when INITB goes high on X_SIM_CONFIG_S3A_SERIAL." severity error;
       end if;
    end if;
   end if;
  end process;
  ...
  process (m_in) begin
    if (mode_sample_flag = '1' and persist_en = '1') then
       assert false report "Error : Mode pine M[2:0] changed after rising edge of INITB on X_SIM_CONFIG_S3A_SERIAL." severity error;
    end if;
  end process;
       ...
       if (rw_en  =  '1' ) then
           ...
           rd_flag <= '0';
            if (bus_sync_flag  =  '0') then
              tmp_word1 := (tmp_word2(14 downto 0) & ds_in);
                  if (tmp_word1 =  X"AA99") then
                          bus_sync_flag <= '1';
                          new_data_in_flag <= '0';
                          wr_cnt <= 0;
                   else 
...
--                assert false report "Warning : only Type 1 Packet supported on X_SIM_CONFIG_S3A_SERIAL." 
--                 severity warning;
                cmd_wr_flag <= '0';
                type2_flag <= '1';
                conti_data_flag <= '1';
    ...
    begin
      if (rst_intl = '0'  or  abort_flag_rst = '1'  or  csi_b_in  =  '1') then
        ...
        if (abort_dis  =  '0'  and  csi_b_in  =  '0') then
             if (NOW  /=  0 ps) then
               abort_flag_wr <= '1';
               assert false report " Warning : RDWRB changes when CS_B low, which causes Configuration abort on X_SIM_CONFIG_S3A_SERIAL."
               severity warning;
             end if;
        else
           abort_flag_wr <= '0';
