...
-- /___/   /\     Filename : X_RAMB16_S1_S2.vhd
...
entity X_RAMB16_S1_S2 is
  ...
  attribute VITAL_LEVEL0 of
    X_RAMB16_S1_S2 : entity is true;
end x_ramb16_s1_s2;

architecture X_RAMB16_S1_S2_V of X_RAMB16_S1_S2 is

  attribute VITAL_LEVEL0 of
    X_RAMB16_S1_S2_V : architecture is true;

  signal ADDRA_ipd : std_logic_vector(13 downto 0)  := (others => 'X');
  ...
  signal ADDRB_ipd : std_logic_vector(12 downto 0)  := (others => 'X');
  ...
  signal ADDRA_dly    : std_logic_vector(13 downto 0)  := (others => 'X');
  ...
  signal ADDRB_dly    : std_logic_vector(12 downto 0)  := (others => 'X');
  signal CLKB_dly     : std_ulogic                    := 'X';
  signal DIB_dly      : std_logic_vector(1 downto 0) := (others => 'X');
  signal ENB_dly      : std_ulogic                    := 'X';
  signal GSR_CLKB_dly : std_ulogic                    := '0';
  signal SSRB_dly     : std_ulogic                    := 'X';
  signal WEB_dly      : std_ulogic                    := 'X';
  constant length_a : integer := 16384;
  constant length_b : integer := 8192;  
  constant width_a : integer := 1;
  constant width_b : integer := 2;  
...
begin
  ...
  begin
    ...
    ADDRB_DELAY : for i in 12 downto 0 generate
  ...
  VITALBehavior                        : process
    ...
    variable INIT_A_reg         : std_logic_vector (0 downto 0) := "0";
    variable INIT_B_reg         : std_logic_vector (1 downto 0) := "00";
    variable ADDRA_dly_sampled : std_logic_vector(13 downto 0)   := (others => 'X');
    variable ADDRB_dly_sampled : std_logic_vector(12 downto 0)   := (others => 'X');
    variable ADDRESS_A         : integer;
    variable ADDRESS_B         : integer;
    variable DOA_OV_LSB        : integer;
    variable DOA_OV_MSB        : integer;
    variable DOA_zd            : std_logic_vector(0 downto 0);
    variable DOB_OV_LSB        : integer;
    variable DOB_OV_MSB        : integer;
    variable DOB_zd            : std_logic_vector(1 downto 0);
    ...
    variable SRVAL_A_reg            : std_logic_vector (0 downto 0) := "0";
    variable SRVAL_B_reg            : std_logic_vector (1 downto 0) := "00";
  ...
  begin
    if (FIRST_TIME) then
      ...
      if (INIT_A'length > 1) then
        INIT_A_reg(0 downto 0)        := To_StdLogicVector(INIT_A)(0 downto 0);
      else
        INIT_A_reg(INIT_A'length-1 downto 0)         := To_StdLogicVector(INIT_A);
      end if;
      DOA_zd(0 downto 0)                       := INIT_A_reg(0 downto 0);
      DOA  <= DOA_zd;

      if (INIT_B'length > 2) then
        INIT_B_reg(1 downto 0)        := To_StdLogicVector(INIT_B)(1 downto 0);
      else
        INIT_B_reg(INIT_B'length-1 downto 0)         := To_StdLogicVector(INIT_B);
      end if;
      DOB_zd(1 downto 0)                       := INIT_B_reg(1 downto 0);
      DOB  <= DOB_zd;

      if (SRVAL_A'length > 1) then
        SRVAL_A_reg(0 downto 0)        := To_StdLogicVector(SRVAL_A)(0 downto 0);
      else
        SRVAL_A_reg(SRVAL_A'length-1 downto 0)         := To_StdLogicVector(SRVAL_A);
      end if;
      if (SRVAL_B'length > 2) then
        SRVAL_B_reg(1 downto 0)        := To_StdLogicVector(SRVAL_B)(1 downto 0);
      else
                  ...
                  if (wea_dly_sampled /= '0') then
                    ...
                    if ((collision_type = 1) or (collision_type = 3)) then
                      Memory_Collision_Msg
                        (collision_type => Write_A_Write_B,
                         EntityName => "X_RAMB16_S1_S2",
                         InstanceName => x_ramb16_s1_s2'path_name,
                         address_a => addra_dly_sampled,
                         address_b => addrb_dly_sampled
                         );
                    end if;
