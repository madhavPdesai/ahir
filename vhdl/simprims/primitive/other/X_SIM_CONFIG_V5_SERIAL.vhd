...
-- /___/   /\     Filename : X_SIM_CONFIG_V5_SERIAL.vhd
-- \   \  /  \    Timestamp : 
--  \___\/\___\
--
-- Revision:
...
entity X_SIM_CONFIG_V5_SERIAL is
  generic (
...
-- end_timing generic
  ...
  attribute VITAL_LEVEL0 of X_SIM_CONFIG_V5_SERIAL : entity is true;

end X_SIM_CONFIG_V5_SERIAL;


architecture X_SIM_CONFIG_V5_SERIAL_V of X_SIM_CONFIG_V5_SERIAL is
...
function bit_revers8 ( din8 : in std_logic_vector(7 downto 0))
  ...
  signal  buswid_flag  : std_ulogic :=  '1';
   ...
   rdwr_b_in <= '0';
        ...
        begin
          if (DEVICE_ID = X"00000000") then
             assert FALSE report "Attribute Syntax Error : The attribute DEVICE_ID on  X_SIM_CONFIG_V5_SERIAL is not set." severity error;
          end if;
          wait;
        end process;
    ...
    d_out(31 downto 8) <=  X"000000" when (abort_out_en = '1') else outbus(31 downto 8);
    d_out_en <= d_o_en;
...
--   wait on rst_en, init_rst, prog_b_in, iprog_b;
      ...
      if (mode_sample_flag = '0') then
        if(prog_b_t = '1') then
           ...
           if (m_in /= "111") then
              ...
              Write ( Message, string'(" . Only Slave SERIAL  mode M=111 supported on X_SIM_CONFIG_V5_SERIAL."));
              assert false report Message.all severity error;
              DEALLOCATE (Message);
           end if;
        elsif (NOW > 0 ps) then
            assert false report "Error: PROGB is not high when INITB goes high on X_SIM_CONFIG_V5_SERIAL." severity error;
       end if;
    end if;
   end if;
  end process;
  ...
  process (m_in) begin
    if (mode_sample_flag = '1' and persist_en = '1') then
       assert false report "Error : Mode pine M[2:0] changed after rising edge of INITB on X_SIM_CONFIG_V5_SERIAL." severity error;
    end if;
  end process;
...
--     variable prog_pulse_low : time;
  ...
  begin
    if (falling_edge (prog_b_in)) then
     ...
     rw_en <=  '1' when (bus_en  =  '1'  and  done_o = '0') else '0';
   ...
   process (cclk_in, rdwr_b_in) begin
    ...
    bus_sync_p : process (cclk_in, desync_flag)
      ...
      variable tmp_dword1  : std_logic_vector (31 downto 0);
      variable tmp_dword2  : std_logic_vector (31 downto 0);
    begin
      if (desync_flag = '1') then
          ...
          wr_flag <= '1';
          rd_flag <= '0';
          tmp_dword1 := X"00000000";
          tmp_dword2 := X"00000000";
      elsif (rising_edge(cclk_in)) then
       if (rw_en  =  '1' ) then
           wr_flag <= '1';
           rd_flag <= '0';
           if ( bus_sync_flag = '0') then
             tmp_dword1 := (tmp_dword2(30 downto 0) & ds_in);
             if (tmp_dword1 = X"AA995566") then
                     bus_sync_flag <= '1';
                     new_data_in_flag <= '0';
                     tmp_dword2 := X"00000000";
                     pack_in_reg <= X"00000000";
             ...
             pack_in_reg <= (pack_in_reg(30 downto 0) & ds_in);
             if (wr_cnt = 31) then
               wr_cnt <= 0;
               new_data_in_flag <= '1';
             else
    ...
    begin
      if (rst_intl = '0'  or  abort_flag_rst = '1'  or  csi_b_in  =  '1') then
        ...
        if (abort_dis  =  '0'  and  csi_b_in  =  '0') then
             if (NOW  /=  0 ps) then
               abort_flag_wr <= '1';
               assert false report " Warning : RDWRB changes when CS_B low, which causes Configuration abort on X_SIM_CONFIG_V5_SERIAL."
               severity warning;
             end if;
        else
           abort_flag_wr <= '0';
