...
-- /___/   /\     Filename : X_RAMB16_S9_S18.vhd
...
entity X_RAMB16_S9_S18 is
  ...
  attribute VITAL_LEVEL0 of
    X_RAMB16_S9_S18 : entity is true;
end x_ramb16_s9_s18;

architecture X_RAMB16_S9_S18_V of X_RAMB16_S9_S18 is

  attribute VITAL_LEVEL0 of
    X_RAMB16_S9_S18_V : architecture is true;
  ...
  signal ADDRA_dly    : std_logic_vector(10 downto 0)  := (others => 'X');
  signal CLKA_dly     : std_ulogic                    := 'X';
  signal DIA_dly      : std_logic_vector(7 downto 0) := (others => 'X');
  signal DIPA_dly     : std_logic_vector(0 downto 0)  := (others => 'X');
...
begin
  ...
  VITALBehavior                        : process
    ...
    variable INIT_A_reg         : std_logic_vector (8 downto 0) := "000000000";
    variable INIT_B_reg         : std_logic_vector (17 downto 0) := "000000000000000000";
    variable ADDRA_dly_sampled : std_logic_vector(10 downto 0)   := (others => 'X');
    ...
    variable DOA_zd            : std_logic_vector(7 downto 0);
    ...
    variable DOPA_zd           : std_logic_vector(0 downto 0);
    ...
    variable SRVAL_A_reg            : std_logic_vector (8 downto 0) := "000000000";
  ...
  begin
    if (FIRST_TIME) then
      ...
      if (INIT_A'length > 9) then
        INIT_A_reg(8 downto 0)        := To_StdLogicVector(INIT_A)(8 downto 0);
      else
        INIT_A_reg(INIT_A'length-1 downto 0)         := To_StdLogicVector(INIT_A);
      end if;
      DOA_zd(7 downto 0)                       := INIT_A_reg(7 downto 0);
      DOPA_zd(0 downto 0)                       := INIT_A_reg(8 downto 8);
      DOA  <= DOA_zd;
      DOPA <= DOPA_zd;
      ...
      if (SRVAL_A'length > 9) then
        SRVAL_A_reg(8 downto 0)        := To_StdLogicVector(SRVAL_A)(8 downto 0);
      else
    ...
    if (GSR_CLKA_dly = '1') then
      DOA_zd  := INIT_A_reg ( 7 downto 0);
      DOPA_zd := INIT_A_reg ( 8 downto 8);
      ...
      if (ena_dly_sampled = '1') then
        if (rising_edge(CLKA_dly)) then
          ...
          if (ssra_dly_sampled = '1') then
            ...
            DOA_zd  := SRVAL_A_reg(7 downto 0);
            DOPA_zd := SRVAL_A_reg(8 downto 8);
          end if;
        end if;
      end if;
    end if;
                  ...
                  if (wea_dly_sampled /= '0') then
                    ...
                    if ((collision_type = 1) or (collision_type = 3)) then
                      Memory_Collision_Msg
                        (collision_type => Write_A_Write_B,
                         EntityName => "X_RAMB16_S9_S18",
                         InstanceName => x_ramb16_s9_s18'path_name,
                         address_a => addra_dly_sampled,
                         address_b => addrb_dly_sampled
                         );
                    end if;
