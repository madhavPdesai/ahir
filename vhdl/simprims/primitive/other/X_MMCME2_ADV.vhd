...
-- begining timing generic
        ...
        tpd_RST_LOCKED : VitalDelayType01 := (100 ps, 100 ps);
...
architecture X_MMCME2_ADV_V of X_MMCME2_ADV is
  ...
  if (clkdiv >= 1) then
     ...
     if (clk_dly_int > 63) then
        ...
        Write ( Message, string'(" of X_MMCME2_ADV is set to "));
        Write ( Message, clk_ps);
        Write ( Message, string'(". Required phase shifting can not be reached since it is over the maximum phase shifting ability of X_MMCME2_ADV"));
        Write ( Message, '.' & LF );
  ...
  begin
    if (di_in(12) /= '1')  then
      Write ( Message, string'(" Error : X_MMCME2_ADV input DI(15 downto 0) is set to"));
...
--        constant CLKPFD_FREQ_MIN : real := 10.0;
  ...
  begin
        ...
        simprim.VCOMPONENTS.PLL_LOCKG <= '0' when (STARTUP_WAIT_BINARY = '1' and locked_out_tmp = '0') else 'H';
...
--       lock_cnt_max <= lock_en_tmp3_int +  clkout_en_time_i1 + 16;
    ...
    begin
      ...
      if (NOW > 1 ps  and  rst_in = '0' and (clkinsel_tmp = '0' or clkinsel_tmp = '1')) then
          assert false report
            "Input Error : X_MMCME2_ADV input clock can only be switched when RST=1.  CLKINSEL is changed when RST low, which should be changed at RST high."
          severity error;
      end if;
...
--  end if;
  ...
  case clkfb_div_fint is
    ...
    if (den_in = '1') then
       ...
       if (drp_lock = '1') then
          assert false report " Warning : DEN is high at X_MMCME2_ADV before DRDY high . Need wait for DRDY signal before next read/write operation through DRP. " severity  warning;
       else 
          drp_lock <= '1';
       end if;
       ...
       if (valid_daddr and ( address = 116  or address = 78 or address = 79 or
         ...
         Write ( Message, string'(" on the X_MMCME2_ADV instance is unsupported") );
         Write ( Message, '.' & LF );
         assert false report Message.all severity warning;
         DEALLOCATE (Message);
       end if;
          ...
          if (rst_input = '1') then
             ...
             if (clkfb_frac_en_v = 0) then
              ...
              clkfbm1_div1 <= clk_div1;
                ...
                assert false report " Input Error : The sum of DI[11:6] and DI[5:0] Address DADDR=0010100 is CLKFBOUT clock divider of X_MMCME2_ADV and over the 2 to 64 range." severity error;
                end if;
             end if;
              clkout_pm_cal(clk_ht1, clk_div, clk_div1, clkfbm2_hti, clkfbm2_lti, clkfbm2_nocnti, clkfbm2_edgei);
              clkfbm2_ht1 <= clk_ht1;
              ...
              if (clk_div > 106 or (clk_div < 1 and clkind_nocnti = '0')) then
                assert false report " Input Error : The sum of DI[11:6] and DI[5:0] Address DADDR=0010110 is input clock divider of X_MMCME2_ADV and over the 1 to 106 range." severity error;
              end if;

          else 
              assert false report " Error : RST is low at X_MMCME2_ADV. RST need to be high when change X_MMCME2_ADV paramters through DRP. " severity error;
          end if; -- end rst
    ...
    begin
      ...
      if (rst_in = '1') then
        ...
        if (fps_en = 1)  then
          if (psen_in = '1')  then
            if (psen_w = '1') then
              assert false report " Error : PSEN on X_MMCME2_ADV is active more than 1 PSCLK period. PSEN must be active for only one PSCLK period. "
              severity warning;
            end if;
            psen_w <= '1';
            if (ps_lock = '1') then
...
--            if (clkout_en = '1' and clkfb_frac_en = 0) then
  ...
  begin
    if (rst_in = '1') then
       ...
       if (clkin_hold_f = 1) then
         ...
         assert FALSE  report " Error : input CLKIN1 of X_MMCME2_ADV is stopped. Reset is required for X_MMCME2_ADV whether stopped input clock comes back or not." severity error;
         else
         assert FALSE report " Error : input CLKIN2 of X_MMCME2_ADV  is stopped. Reset is required for X_MMCME2_ADV whether stopped input clock comes back or not." severity error;
         end if;
       end if;
    end if;
  ...
  begin
    ...
    VitalPathDelay01 (
      ...
      Paths => (0 => (RST_dly'last_event, tpd_RST_LOCKED, true)),
      Mode => VitalTransport,
      Xon => Xon,
      MsgOn => MsgOn,
      MsgSeverity => warning
  ...
  begin
    if (TimingChecksOn) then
     ...
     VitalSetupHoldCheck
       (
         ...
         CheckEnabled   => TRUE,
         RefTransition  => 'R',
         HeaderMsg      => InstancePath & "/X_MMCME2_ADV",
         Xon            => Xon,
         MsgOn          => MsgOn,
         MsgSeverity    => WARNING
       );
    ...
    wait on  CLKIN1_ipd, CLKIN2_ipd, RST_ipd, DCLK_dly, DEN_dly, DWE_dly, DI_dly, DADDR_dly, PSEN_dly, PSINCDEC_dly;
