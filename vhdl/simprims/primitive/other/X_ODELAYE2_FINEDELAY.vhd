...
-- /___/   /\     Filename : X_ODELAYE2_FINEDELAY.vhd
...
--  VITAL input Pin path delay variables
      ...
      tipd_OFDLY        : VitalDelayArrayType01 (2 downto 0) := (others => (0 ps, 0 ps));
...
-- VITAL removal time variables
  ...
  attribute VITAL_LEVEL0 of
    X_ODELAYE2_FINEDELAY : entity is true;

end X_ODELAYE2_FINEDELAY;

architecture X_ODELAYE2_FINEDELAY_V OF X_ODELAYE2_FINEDELAY is

  attribute VITAL_LEVEL0 of
    X_ODELAYE2_FINEDELAY_V : architecture is true;
...
begin
  ...
  begin
    ...
    OFDLY_DELAY : for i in 0 to 2 generate
       VitalWireDelay (OFDLY_ipd(i),OFDLY(i),tipd_OFDLY(i));
    end generate OFDLY_DELAY;
  end block;
...
--####################################################################
  ...
  begin
     ...
     if((CINVCTRL_SEL /= "TRUE") and (CINVCTRL_SEL /= "FALSE"))then
          assert false
          report "Attribute Syntax Error: The attribute CINVCTRL_SEL on X_ODELAYE2_FINEDELAY must be set to either TRUE or FALSE."
          severity Failure;
     end if;
    ...
    --------DELAY_SRC check
     if((DELAY_SRC /= "ODATAIN") and (DELAY_SRC /= "CLKIN"))then
          assert false
          report "Attribute Syntax Error: The attribute DELAY_SRC on X_ODELAYE2_FINEDELAY must be set to either ODATAIN or CLKIN."
          severity Failure;
     end if;
     ...
     if((HIGH_PERFORMANCE_MODE /= "TRUE") and (HIGH_PERFORMANCE_MODE /= "FALSE"))then
          assert false
          report "Attribute Syntax Error: The attribute HIGH_PERFORMANCE_MODE on X_ODELAYE2_FINEDELAY must be set to either TRUE or FALSE."
          severity Failure;
     end if;
     ...
     if(ODELAY_TYPE = "FIXED") then
       ...
       (  HeaderMsg  => " Attribute Syntax Warning ",
          ...
          EntityName => "/X_ODELAYE2_FINEDELAY",
     ...
     if (TimingChecksOn) then
       VitalSetupHoldCheck
         (
           ...
           RefTransition  => 'R',
           HeaderMsg      => InstancePath & "/X_ODELAYE2_FINEDELAY",
           Xon            => Xon,
           MsgOn          => MsgOn,
           MsgSeverity    => WARNING
        );
