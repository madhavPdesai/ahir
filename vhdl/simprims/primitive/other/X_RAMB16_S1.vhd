-- $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/vhdsclibs/data/simprims/simprim/VITAL/X_RAMB16_S1.vhd,v 1.3 2010/01/14 19:43:23 fphillip Exp $
-------------------------------------------------------------------------------
...
entity X_RAMB16_S1 is
  ...
  attribute VITAL_LEVEL0 of
    X_RAMB16_S1 : entity is true;
end X_RAMB16_S1;

architecture X_RAMB16_S1_V of X_RAMB16_S1 is

  attribute VITAL_LEVEL0 of
    X_RAMB16_S1_V : architecture is true;

  signal ADDR_ipd : std_logic_vector(13 downto 0) := (others => 'X');
  ...
  signal ADDR_dly : std_logic_vector(13 downto 0) := (others => 'X');
  signal CLK_dly  : std_ulogic                    := 'X';
  signal DI_dly   : std_logic_vector(0 downto 0)  := (others => 'X');
  signal EN_dly   : std_ulogic                    := 'X';
  signal SSR_dly  : std_ulogic                    := 'X';
  signal GSR_dly  : std_ulogic := '0';
  signal WE_dly   : std_ulogic                    := 'X';

  constant length : integer := 16384;
...
begin
  ...
  WireDelay    : block
  begin
    ADDR_DELAY : for i in 13 downto 0 generate
      VitalWireDelay (ADDR_ipd(i), ADDR(i), tipd_ADDR(i));
    end generate ADDR_DELAY;
    VitalWireDelay (CLK_ipd, CLK, tipd_CLK);
    DI_DELAY   : for i in 0 downto 0 generate
  ...
  VITALBehavior                       : process
    ...
    variable Tviol_ADDR12_CLK_posedge : std_ulogic := '0';
    variable Tviol_ADDR13_CLK_posedge : std_ulogic := '0';
    ...
    variable Tmkr_ADDR12_CLK_posedge : VitalTimingDataType := VitalTimingDataInit;
    variable Tmkr_ADDR13_CLK_posedge : VitalTimingDataType := VitalTimingDataInit;
    ...
    variable DO_zd          : std_logic_vector(0 downto 0)              := INI(0 downto 0 );
  ...
  begin
    ...
    if (TimingChecksOn) then
      ...
      VitalSetupHoldCheck (
        ...
        MsgSeverity    => warning);
      VitalSetupHoldCheck (
        Violation      => Tviol_ADDR13_CLK_posedge,
        TimingData     => Tmkr_ADDR13_CLK_posedge,
        TestSignal     => ADDR_dly(13),
        TestSignalName => "ADDR(13)",
        TestDelay      => tisd_ADDR_CLK(13),
        RefSignal      => CLK_dly,
        RefSignalName  => "CLK",
        RefDelay       => ticd_CLK,
        SetupHigh      => tsetup_ADDR_CLK_posedge_posedge(13),
        SetupLow       => tsetup_ADDR_CLK_negedge_posedge(13),
        HoldLow        => thold_ADDR_CLK_posedge_posedge(13),
        HoldHigh       => thold_ADDR_CLK_negedge_posedge(13),
        CheckEnabled   => TO_X01(EN_dly) /= '0',
        RefTransition  => 'R',
        HeaderMsg      => "/X_RAMB16_S1",
        Xon            => Xon,
        ...
        MsgSeverity    => warning);
      VitalSetupHoldCheck (
        Violation      => Tviol_ADDR12_CLK_posedge,
        TimingData     => Tmkr_ADDR12_CLK_posedge,
        TestSignal     => ADDR_dly(12),
        TestSignalName => "ADDR(12)",
        TestDelay      => tisd_ADDR_CLK(12),
        RefSignal      => CLK_dly,
        RefSignalName  => "CLK",
        RefDelay       => ticd_CLK,
        SetupHigh      => tsetup_ADDR_CLK_posedge_posedge(12),
        SetupLow       => tsetup_ADDR_CLK_negedge_posedge(12),
        HoldLow        => thold_ADDR_CLK_posedge_posedge(12),
        HoldHigh       => thold_ADDR_CLK_negedge_posedge(12),
        CheckEnabled   => TO_X01(EN_dly) /= '0',
        RefTransition  => 'R',
        HeaderMsg      => "/X_RAMB16_S1",
        Xon            => Xon,
    ...
    Violation :=
      ...
      Tviol_ADDR12_CLK_posedge or
      Tviol_ADDR13_CLK_posedge or
    ...
    if (GSR_dly = '1') then
      DO_zd := INI(0 downto 0 );
    elsif (GSR_dly = '0') then
      if (rising_edge(CLK_dly)) then
        if (EN_dly = '1') then
          if (SSR_dly = '1') then
            DO_zd := To_StdLogicVector(SRVAL)(0 downto 0);
          else    
            if (WE_dly = '1') then
              if (wr_mode = 0) then
                DO_zd := DI_dly;
    ...
    wait on ADDR_dly, CLK_dly, DI_dly, EN_dly, GSR_dly, SSR_dly, WE_dly;
