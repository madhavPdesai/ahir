-- $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/vhdsclibs/data/simprims/blanc/VITAL/X_FIFO18E1.vhd,v 1.44 2012/03/09 22:38:51 wloo Exp $
-------------------------------------------------------------------------------
  ...
  begin
     if (first_time) then
       ...
       if(fwft_var = '1' and EN_SYN = TRUE) then
          assert false
          report "DRC Error : First word fall through is not supported in synchronous mode on X_FIFO18E1."
          severity failure;
       end if;
       ...
       if(EN_SYN = FALSE and DO_REG = 0) then
          assert false
          report "DRC Error : DO_REG = 0 is invalid when EN_SYN is set to FALSE on X_FIFO18E1."
          severity failure;
       end if;
       ...
       if ((EN_ECC_WRITE = TRUE or EN_ECC_READ = TRUE) and DATA_WIDTH /= 72) then
          assert false
            report "DRC Error : The attribute DATA_WIDTH must be set to 72 when X_FIFO18E1 is configured in the ECC mode."
          severity failure;
       end if;
              ...
              if (period_rdclk_var = period_wrclk) then
                      ...
                      if (SIM_DEVICE = "7SERIES") then
                          ...
                          if (fwft = '0') then
                              ...
                              if ((aempty_offset_int < 5) or (aempty_offset_int > addr_limit - 6)) then
                                ...
                                write( Message, STRING'("ALMOST_EMPTY_OFFSET on X_FIFO18E1 is set to ") );
                              ...
                              if ((afull_offset_int < 4) or (afull_offset_int > addr_limit - 7)) then
                                write( Message, STRING'("Attribute Syntax Error : ") );
                                write( Message, STRING'("The attribute ") );
                                write( Message, STRING'("ALMOST_FULL_OFFSET on X_FIFO18E1 is set to ") );
                ...
                if (SIM_DEVICE = "7SERIES") then
                  ...
                  if (afull_offset_int > (addr_limit - (s7_roundup_period_rd_wr_int + 6))) then
                    ...
                    write( Message, STRING'("The attribute ") );
                    write( Message, STRING'("ALMOST_FULL_OFFSET on AX_FIFO18E1 is set to ") );
                    write( Message, afull_offset_int);
                    write( Message, STRING'(". It must be set to a value smaller than (FIFO_DEPTH - ((roundup (4 * (WRCLK frequency / RDCLK frequency))) + 6)) when X_FIFO18E1 has different frequencies for RDCLK and WRCLK.") );
                    ASSERT FALSE REPORT Message.ALL SEVERITY Failure;
                    DEALLOCATE (Message);
                  end if;
                  ...
                  if (afull_offset_int > (addr_limit - ((3 * roundup_period_wr_rd_int) + 3))) then
                    ...
                    write( Message, STRING'("ALMOST_FULL_OFFSET on AX_FIFO18E1 is set to ") );
                    write( Message, afull_offset_int);
                    write( Message, STRING'(". It must be set to a value smaller than (FIFO_DEPTH - ((3 * roundup (RDCLK frequency / WRCLK frequency)) + 3)) when X_FIFO18E1 has different frequencies for RDCLK and WRCLK.") );
                    ASSERT FALSE REPORT Message.ALL SEVERITY Failure;
                    DEALLOCATE (Message);
                  end if;
                  ...
                  if (aempty_offset_int > (addr_limit - ((3 * roundup_period_rd_wr_int) + 3))) then
                    write( Message, STRING'("DRC Error : ") );
                    write( Message, STRING'("The attribute ") );
                    write( Message, STRING'("ALMOST_EMPTY_OFFSET on AX_FIFO18E1 is set to ") );
                    write( Message, aempty_offset_int);
                    write( Message, STRING'(". It must be set to a value smaller than (FIFO_DEPTH - ((3 * roundup (WRCLK frequency / RDCLK frequency)) + 3)) when X_FIFO18E1 has different frequencies for RDCLK and WRCLK.") );
                    ASSERT FALSE REPORT Message.ALL SEVERITY Failure;
                    DEALLOCATE (Message);
                  end if;
      ...
      if(rising_edge(WRCLK_dly)) then
        ...
        if (not(EN_ECC_WRITE = TRUE or EN_ECC_READ = TRUE)) then
          ...
          if (injectsbiterr_dly = '1') then
            assert false
              report "DRC Warning : INJECTSBITERR is not supported when neither EN_ECC_WRITE nor EN_ECCREAD = TRUE on X_FIFO18E1 instance."
              severity Warning;
          end if;

          if (injectdbiterr_dly = '1') then
            assert false
            report "DRC Warning : INJECTDBITERR is not supported when neither EN_ECC_WRITE nor EN_ECCREAD = TRUE on X_FIFO18E1 instance."
            severity Warning;
          end if;
