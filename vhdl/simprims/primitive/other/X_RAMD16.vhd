-- $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/vhdsclibs/data/simprims/simprim/VITAL/X_RAMD16.vhd,v 1.3 2010/01/14 19:43:23 fphillip Exp $
-------------------------------------------------------------------------------
...
entity X_RAMD16 is
  ...
  attribute VITAL_LEVEL0 of
    X_RAMD16 : entity is true;
end X_RAMD16;

architecture X_RAMD16_V of X_RAMD16 is
  attribute VITAL_LEVEL0 of
    X_RAMD16_V : architecture is true;
...
begin
  ...
  VITALReadBehavior     : process(RADR3_ipd, RADR2_ipd, RADR1_ipd, RADR0_ipd, MEM)
    variable index_rd   : integer := 16 ;
    variable rd_address : std_logic_vector (3 downto 0);
  begin
    rd_address                    := (RADR3_ipd, RADR2_ipd, RADR1_ipd, RADR0_ipd);
    index_rd                      := SLV_TO_INT(SLV => rd_address);
    Q_zd <= MEM(index_RD);
  end process VITALReadBehavior;

  VITALWriteBehavior    : process(CLK_dly, violation)
    variable index_wr   : integer := 16;
    variable wr_address : std_logic_vector( 3 downto 0);
  begin
    if (rising_edge(CLK_dly)) then
      if (WE_dly = '1') then
        wr_address                := (WADR3_dly, WADR2_dly, WADR1_dly, WADR0_dly);
  ...
  VITALTimingCheck                   : process (CLK_dly, I_dly, WADR0_dly, WADR1_dly, WADR2_dly, WADR3_dly, WE_dly)
  ...
  begin
    ...
    VitalPathDelay01 (
      ...
      OutSignalName => "O",
      OutTemp       => O_zd,
      Paths         => (0 => (CLK_dly'last_event, tpd_CLK_O, (WE_dly /= '0') and (RADR0_ipd = WADR0_dly and RADR1_ipd = WADR1_dly and RADR2_ipd = WADR2_dly and RADR3_ipd = WADR3_dly)),
