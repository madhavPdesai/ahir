...
-- /___/   /\     Filename : X_IDELAYE2_FINEDELAY.vhd
...
--  VITAL input Pin path delay variables
      ...
      tipd_IFDLY   	: VitalDelayArrayType01 (2 downto 0) := (others => (0 ps, 0 ps));
...
-- VITAL removal time variables
  ...
  attribute VITAL_LEVEL0 of
    X_IDELAYE2_FINEDELAY : entity is true;

end X_IDELAYE2_FINEDELAY;

architecture X_IDELAYE2_FINEDELAY_V OF X_IDELAYE2_FINEDELAY is

  attribute VITAL_LEVEL0 of
    X_IDELAYE2_FINEDELAY_V : architecture is true;
...
begin
  ...
  begin
    ...
    IFDLY_DELAY : for i in 0 to 2 generate
       VitalWireDelay (IFDLY_ipd(i),IFDLY(i),tipd_IFDLY(i));
    end generate IFDLY_DELAY;
  end block;
...
--####################################################################
  ...
  begin
     ...
     if((CINVCTRL_SEL /= "TRUE") and (CINVCTRL_SEL /= "FALSE"))then
          assert false
          report "Attribute Syntax Error: The attribute CINVCTRL_SEL on X_IDELAYE2_FINEDELAY must be set to either TRUE or FALSE."
          severity Failure;
     end if;
     ...
     if((HIGH_PERFORMANCE_MODE /= "TRUE") and (HIGH_PERFORMANCE_MODE /= "FALSE"))then
          assert false
          report "Attribute Syntax Error: The attribute HIGH_PERFORMANCE_MODE on X_IDELAYE2_FINEDELAY must be set to either TRUE or FALSE."
          severity Failure;
     end if;
     ...
     if(IDELAY_TYPE = "FIXED") then
       ...
       (  HeaderMsg  => " Attribute Syntax Warning ",
          ...
          EntityName => "/X_IDELAYE2_FINEDELAY",
     ...
     if((PIPE_SEL /= "TRUE") and (PIPE_SEL /= "FALSE"))then
          assert false
          report "Attribute Syntax Error: The attribute PIPE_SEL on X_IDELAYE2_FINEDELAY must be set to either TRUE or FALSE."
          severity Failure;
     end if;
...
--####################################################################
  ...
  begin
      if(DELAY_SRC = "IDATAIN") then 
         ...
         assert false
         report "Attribute Syntax Error : Legal values for DELAY_SRC on X_IDELAYE2_FINEDELAY instance are DATAIN or IDATAIN."
         severity Failure;
      end if;
     ...
     if (TimingChecksOn) then
       VitalSetupHoldCheck
         (
           ...
           RefTransition  => 'R',
           HeaderMsg      => InstancePath & "/X_IDELAYE2_FINEDELAY",
           Xon            => Xon,
           MsgOn          => MsgOn,
           MsgSeverity    => WARNING
        );
