...
_sum_mod_ (bit_vector * __pa, bit_vector * __pb, bit_vector * __pc)
{
  ...
  {
//              q := (a + b) $buffering 1// bits of buffering = 10. 
    _sum_mod_assign_stmt_42_c_macro_;
//              r := (q + b) $buffering 1// bits of buffering = 10. 
...
//      TM := ($bitcast ($uint<32>) (r && b) ) $buffering 1// bits of buffering = 32. 
  _sum_mod_assign_stmt_59_c_macro_;
//      M := ($bitcast ($uint<20>) TM ) $buffering 1// bits of buffering = 20. 
  _sum_mod_assign_stmt_63_c_macro_;
//      c := ( $mux ($bitcast ($uint<1>) 1  ) M  M )  $buffering 1// bits of buffering = 20. 
  _sum_mod_assign_stmt_71_c_macro_;
...
//      aaa[0 ] := ($bitcast ($uint<10>) a ) $buffering 1// bits of buffering = 10. 
  _sum_mod_wrap_assign_stmt_16_c_macro_;
//      aaa[1 ] := ($bitcast ($uint<10>) b ) $buffering 1// bits of buffering = 10. 
  _sum_mod_wrap_assign_stmt_21_c_macro_;
//      $call sum_mod (aaa[0 ] aaa[1 ] ) (c20 ) 
  _sum_mod_wrap_call_stmt_27_c_macro_;
//      c := ($bitcast ($uint<32>) c20 ) $buffering 1// bits of buffering = 32. 
