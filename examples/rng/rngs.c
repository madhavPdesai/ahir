...
static int32_t seed[STREAMS];  /* current state of each stream   */
 ...
 */
{
  ...
  t = MULTIPLIER * srem32(seed[stream],Q) - R * sdiv32(seed[stream] , Q);
  if (t > 0) 
    seed[stream] = t;
  else 
 ...
 */
{
  const int32_t Q = MODULUS / A256;
  const int32_t R = MODULUS % A256;
        int  j;
        int  s;
  ...
  for (j = 1; j < STREAMS; j++) {
    x = A256 * srem32(seed[j - 1] , Q) - R * sdiv32(seed[j - 1] , Q);
    if (x > 0)
      seed[j] = x;
    else
      seed[j] = x + MODULUS;
 ...
 */
{
  ...
  x = srem32(x , MODULUS);                       /* correct if x is too large  */
  seed[stream] = x;
}
 ...
 */
{
  stream =  srem32(((unsigned int) index) , STREAMS);
