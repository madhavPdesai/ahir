...
{\bf Aa} is a low-level {\em algorithm-assembly}
programming language for the description
of algorithms.   An \Aa program 
...
(set to 32 for now), which specified the
...
\end{verbatim}
is an two-dimensional array type whose
...
int b = *((int*) &a);
\end{verbatim}
except that in \Aa, the destination type need not
...
storage objects need to be grouped into memory spaces using
the following rule: two storage objects are put in the same
memory space if there is a value in the program which
is infected by (determined by) the values of pointers to the two objects.
...
\end{verbatim}
Now consider the object {\bf ptr}, which can hold a value which
...
the same memory space.  In this case, the external
object needs
