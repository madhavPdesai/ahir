...
void Print_C_Pipe_Registration(string pipe_name, AaType* pipe_type, int  depth, bool port_mode, bool in_mode, bool lifo_mode, ofstream& ofile)
{
	...
	if(port_mode)
	{
		...
		ofile << "register_port(\"" << pipe_name << "\", " <<  wsize << ", " <<   (in_mode ? 1 : 0) << ");" << endl;
	}
	else
	{
		ofile << "register_pipe(\"" << pipe_name << "\", " <<  (nwords*depth)
			<< ", " << wsize << ", " <<   (lifo_mode ? 1 : 0) << ");" << endl;
...
void Print_C_Pipe_Read(string tgt, AaType* tgt_type, AaPipeObject* p, ofstream& ofile)
{
	...
	if(tgt_type->Is_Integer_Type() || tgt_type->Is_Pointer_Type())
	{
		if((tsize == 8) || (tsize == 16) || (tsize == 32) || (tsize == 64))
		{
			...
			{
				ofile << tgt << " = (" << tgt_type->C_Name() << "*)  read_pointer(\"" <<  p->Get_Name() << "\");" << endl;
			}
			else
			{
		...
		else
		{
			int wsize = 8;
			int nwords = ((tsize % wsize) ? (tsize/wsize)+1 : tsize/wsize);
			ofile  << "read_uint8_n(\"" << p->Get_Name() << "\", " << tgt << ".val.byte_array," << tgt << ".val.array_size);" << endl;
		}
	}
	else if(tgt_type->Is_Float_Type())
	{
		ofile << tgt <<  " = read_float" << tsize << "(\"" << p->Get_Name() << "\"); " << endl;
	}
}
...
void Print_C_Pipe_Write(string src, AaType* src_type, AaPipeObject* p, ofstream& ofile)
{
	...
	if(src_type->Is_Integer_Type() || src_type->Is_Pointer_Type())
	{
		if( (tsize == 8) || (tsize == 16) || (tsize == 32) || (tsize == 64) )
		{
			...
			{
				ofile << "write_pointer" << tsize << "(\"" << p->Get_Name() << "\", (void*) " << src << "); " << endl;
			}
			else
			{
				ofile << "{ " << endl;
	...
	switch(op)
	{
		...
		case __EQUAL:
			...
			else
			{
				...
				ofile << "bit_vector_assign_uint64(0" <<  ", &(" << tgt << "), " << op_string << ");" << endl;
			}
			break;
		case __NOTEQUAL:
			if(src1_is_int)
