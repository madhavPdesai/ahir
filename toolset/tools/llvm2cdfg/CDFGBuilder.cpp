...
CDFGBuilder::CDFGBuilder(llvm::TargetData *_TD, llvm::AliasAnalysis *_AA)
{
  clear();
  TD = _TD;
  AA = new AccessAnalysis(_AA, TD, this);
  program = NULL;
};

/* ---- Initialisation ---- */

void CDFGBuilder::clear()
{
  cdfg = NULL;
  bb = NULL;
  bbnode = NULL;
...
void CDFGBuilder::create_cdfg(llvm::Function &F)
{
...
CDFGNode* CDFGBuilder::create_acceptor_node(llvm::Function &F)
{
...
void CDFGBuilder::initialise_with_function(llvm::Function &F)
{
...
void CDFGBuilder::finalise_function()
{
...
void CDFGBuilder::visitBasicBlock(BasicBlock &BB)
{
  set_bb(BB);
  AA->clear();
...
void CDFGBuilder::visitInstruction(Instruction &I)
{
  assert(!isa<GetElementPtrInst>(&I) && "GEP should not reach here");
  assert(false && "unsupported instruction");
}

void CDFGBuilder::visitBinaryOperator(BinaryOperator &I)
{
  NodeType ntype;
...
void CDFGBuilder::visitReturnInst(ReturnInst &R)
{
...
void CDFGBuilder::visitAllocaInst(AllocaInst &I)
{
...
void CDFGBuilder::phi_connect_input(CDFGNode *mux
...
void CDFGBuilder::visitPHINode(PHINode &P)
{
...
void CDFGBuilder::visitCallInst(CallInst &C)
{
...
NodeType CDFGBuilder::getAppropriateCast(CastInst &C)
{
...
void CDFGBuilder::visitCastInst(CastInst& C)
{
  const llvm::Type *dest = C.getDestTy();
  llvm::Value *val = C.getOperand(0);
...
void CDFGBuilder::visitLoadInst(LoadInst &L)
{
...
void CDFGBuilder::visitStoreInst(StoreInst &S)
{
...
void CDFGBuilder::visitCmpInst(CmpInst &C)
{
  AA->handleGenericInstruction(&C);
...
void CDFGBuilder::visitSelectInst(SelectInst &S)
{
...
void CDFGBuilder::handle_terminator_inst()
{
...
void CDFGBuilder::visitBranchInst(BranchInst &br)
{
...
BBNode* CDFGBuilder::create_bbnode(BasicBlock &bb)
{
...
void CDFGBuilder::set_bb(BasicBlock &BB)
{
  bb = &BB;
  bbnode = get_bbnode(BB);
}

void CDFGBuilder::register_bbnode(BasicBlock &BB, BBNode *bbnode)
{
...
BBNode* CDFGBuilder::get_bbnode(BasicBlock *BB)
{
  assert(blocks.find(BB) != blocks.end()
...
void CDFGBuilder::register_node(llvm::Value *val
				, CDFGNode *node)
{
...
CDFGNode* CDFGBuilder::get_node(llvm::Value &val)
{
  return get_node(&val);
}
...
void CDFGBuilder::create_edge(Port *port)
{
...
Port* CDFGBuilder::create_output_data_port(CDFGNode *node
					   , const std::string &id
...
Port* CDFGBuilder::create_input_data_port(CDFGNode *node
					  , const std::string &id
					  , const llvm::Type *type)
{
...
Port* CDFGBuilder::create_output_control_port(CDFGNode *node
					      , const std::string &id)
{
...
Port* CDFGBuilder::create_input_control_port(CDFGNode *node
					     , const std::string &id)
{
...
CDFGNode* CDFGBuilder::create_node(NodeType t, const std::string &d)
{
...
CDFGNode* CDFGBuilder::create_data_node(NodeType t
					, const llvm::Type *type
					, const std::string &d)
{
...
Addressable* CDFGBuilder::create_addressable(AllocaInst &I)
{
...
Port* CDFGBuilder::get_output_data_port(CDFGNode *vnode, llvm::Value *val)
{
  Port *vport = NULL;
  switch (vnode->ntype) {
    case Accept: {
...
bool CDFGBuilder::handleOperand(CDFGNode *node
				, llvm::Value *val
...
void CDFGBuilder::connect_to_exit(Instruction *I)
{
  CDFGNode *node = get_node(I);
  connect_to_exit(node, *I);
}

void CDFGBuilder::connect_to_exit(CDFGNode *node, Instruction &I)
{
...
void CDFGBuilder::register_pending_user(llvm::Value *val, Port *port)
{
...
void CDFGBuilder::clear_pending_users(llvm::Value &val, CDFGNode *node)
{
...
void CDFGBuilder::create_pred_join(CDFGNode *node)
{
...
void CDFGBuilder::initialise_control_flow(CDFGNode *node)
{
...
void CDFGBuilder::create_succ_fork(CDFGNode *node)
{
...
void CDFGBuilder::connect_entry_to_node(CDFGNode *node)
{
  CDFGNode *join = joins[node];
...
void CDFGBuilder::control_flow(llvm::Instruction *pred, llvm::Instruction *succ)
{
...
void CDFGBuilder::control_flow(CDFGNode *src
...
void CDFGBuilder::control_flow(CDFGNode *src, CDFGNode *dst)
{
...
void CDFGBuilder::control_flow(Port *src
			       , CDFGNode *dst
			       , const std::string &port_at_dst)
{
...
void CDFGBuilder::connect_ports(Port *src, Port *dst)
{
...
void CDFGBuilder::connect_to_block(CDFGNode *node, BasicBlock *dest, const std::string &dest_id)
{
