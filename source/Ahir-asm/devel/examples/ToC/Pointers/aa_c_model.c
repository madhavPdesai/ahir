...
// -------------------------------------------------------------------------------------------
      if (__top->_assign_line_14_entry)
	{
	  if (1)
	    {
	      (__top->incr_b).__val = __PLUS ((__top->incr_temp).__val, 1);	//  file Pointers.aa, line 14
...
// -------------------------------------------------------------------------------------------
      if (__top->_assign_line_22_entry)
	{
	  if (1)
	    {
	      (u).__val = (__top->a).__val;	//  file Pointers.aa, line 22
...
// -------------------------------------------------------------------------------------------
      if (__top->_assign_line_23_entry)
	{
	  if (1)
	    {
	      (__top->pu) = (&(u));	//  file Pointers.aa, line 23
...
// -------------------------------------------------------------------------------------------
      if (__top->_assign_line_24_entry)
	{
	  if (1)
	    {
	      (__top->q).__val = ((uint32_t) (__top->pu));	//  file Pointers.aa, line 24
...
// -------------------------------------------------------------------------------------------
      if (__top->_assign_line_24_exit)
	{
	  ...
	  __top->_call_line_25_entry = 1;
	}
// -------------------------------------------------------------------------------------------
// Begin Statement _call_line_25
// -------------------------------------------------------------------------------------------
      if (__top->_call_line_25_entry)
	{
	  if (__top->_call_line_25_called_fn_struct == NULL)
	    {
	      __top->_call_line_25_called_fn_struct =
		(increment_State *) calloc (1, sizeof (increment_State));
// reset entry flag and set in progress flag
	      __top->_call_line_25_entry = 0;
	      __top->_call_line_25_in_progress = 1;
	    }			// allocation of pointer to called function
	}			// entry into this call statement
      if (__top->_call_line_25_in_progress)
	{
	  if (!__top->_call_line_25_called_fn_struct->increment_entry
	      && !__top->_call_line_25_called_fn_struct->
	      increment_in_progress)
	    {			// entry and in_progress flags not set?
	      if (1)
		{		// check if pipes can be read from
		  __top->_call_line_25_called_fn_struct->incr_a.__val =
		    (__top->q).__val;
		  __top->_call_line_25_called_fn_struct->increment_entry = 1;
		}		// arguments copied to call structure
	    }			// called function had entry flag set
	  if (__top->_call_line_25_called_fn_struct->increment_entry
	      || __top->_call_line_25_called_fn_struct->increment_in_progress)
	    {			// called function still in progress
// call the function
	      __top->_call_line_25_called_fn_struct = increment_ (__top->_call_line_25_called_fn_struct);	//  file Pointers.aa, line 25
	    }			// called function was in progress 
	  if (__top->_call_line_25_called_fn_struct->increment_exit)
	    {
	      if (1)
		{
		  (__top->b).__val =
		    __top->_call_line_25_called_fn_struct->incr_b.__val;
		  cfree (__top->_call_line_25_called_fn_struct);
		  __top->_call_line_25_called_fn_struct = NULL;
// reset in progress flag and set exit flag
		  __top->_call_line_25_exit = 1;
		  __top->_call_line_25_in_progress = 0;
		}		// ok to copy outputs to destinations?
	    }			// called function had finishes?
	}			// statement was in progress
// -------------------------------------------------------------------------------------------
// End Statement _call_line_25
// -------------------------------------------------------------------------------------------
      if (__top->_call_line_25_exit)
	{
	  __top->_assign_line_22_exit = 0;
	  __top->_assign_line_23_exit = 0;
	  __top->_assign_line_24_exit = 0;
	  __top->_call_line_25_exit = 0;
