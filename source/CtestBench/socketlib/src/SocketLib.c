...
void send_packet_and_wait_for_response(char* buffer, char* server_host_address, int server_port_number)
{
  ...
  if (server == NULL) 
	...
	n=connect(sockfd,(struct sockaddr*) &serv_addr,sizeof(serv_addr));
      ...
      write(n,buffer, strlen(buffer)+1);
      fprintf(stderr, "Info: sent message %s to server", buffer);
 
      int nbytes = read(n,buffer,MAX_BUF_SIZE);
      if(nbytes > 0)
	{
	  fprintf(stderr, "Info: received message %s from server", buffer);	  
	}
      else
	{
