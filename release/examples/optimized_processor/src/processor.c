...
void decode()
{
  while(1)
  {
  	...
  	uint16_t offs = (imm > 127 ? (128 - imm) : imm);  /* get the offset, */ 
  ...
  while(1)
  {
	...
	//fprintf(stderr,"pc=%x, op=%x\n", pc, op);
  	...
  	switch( op )
    	...
    	case 0:
      	/* halt the CPU */
      		write_uint16("processor_to_env_halt",1);
...
void run()
{
  ...
  while(1)
  {
	uint16_t pc = read_uint16("env_to_processor_start_pc");
	fprintf(stderr,"INFO: start pc = %x\n", pc);
	write_uint16("next_pc", pc);
  }
}
